  ...
  <vxe-modal v-model="showEdit" :title="selectRow ? '编辑&保存' : '新增&保存'" width="800" min-width="600" min-height="300" :loading="submitLoading" resize destroy-on-close>
    <template #default>
      <vxe-form :data="formData" :rules="formRules" title-align="right" title-width="100" @submit="submitEvent">
        ...
        <vxe-form-item field="groupName" title="小组名称" :span="24" :item-render="{}">
          <template #default="{ data }">
            <vxe-input v-model="data.groupName" placeholder="请输入名称"></vxe-input>
          </template>
        </vxe-form-item>
...
export default {
  ...
  methods: {
    ...
    async removeEvent (row) {
      ...
      if (type === 'confirm') {
        $table.remove(row)
        axios.delete('/api/v1/manage/group',{
          data:{
            groupId:row.groupId
          }
        })
